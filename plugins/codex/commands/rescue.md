...
- The receipt's last non-empty line before the blank line is a `wait <job-id> ...` command (see `node "${CLAUDE_PLUGIN_ROOT}/scripts/codex-companion.mjs" wait --help`). As soon as the `Agent` tool call returns, run that exact command yourself via `Bash(..., run_in_background: true)`.
